...
public:
    ...
    void print_patient_info_from_string(std::string name);
    ...
    std::map<Person*, std::vector<CarePeriod*>> get_careperiods();
...
private:
    ...
    std::map<std::string, Person*> all_people_;

    // Attribuutti joka sisältää kaikki hoitojaksot
    std::map<Person*, std::vector<CarePeriod*>> careperiods_;
    // Attribuutti, joka sisältää kaikki hoitojaksot johon hoitaja on lisätty
    std::map<Person*, std::vector<CarePeriod*>> staffs_careperiods_;
};
